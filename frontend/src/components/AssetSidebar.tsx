import { Button, Drawer, Empty, List, Select, Space, Tag, Tree } from 'antd';
import type { DataNode } from 'antd/es/tree';
import { Settings } from 'lucide-react';
import type { ReactNode } from 'react';
import { useState } from 'react';
import type { RoleEntry, RoleInventory, RoleMapping, TreeItem, WorkbenchContext } from '../types';
...
export function AssetSidebar({
  ...
  return (
    <aside className="asset-sidebar">
      <AssetSection title="制度管理">
        <Tree
          checkable
          defaultExpandAll
          selectable={false}
          treeData={toDataNodes(policyTree, (item) => {
            if (item.key.startsWith('policy_')) onOpenPolicy(item.key);
          })}
          checkedKeys={context.policyIds}
          onCheck={(_, info) => {
            const key = String(info.node.key);
            if (key.startsWith('policy_')) onPolicyToggle(key);
          }}
          onDoubleClick={(_, node) => {
            const key = String(node.key);
            if (key.startsWith('policy_')) onOpenPolicy(key);
          }}
        />
      </AssetSection>
      <AssetSection title="审批流管理">
        <Tree
          checkable
          defaultExpandAll
          selectable={false}
          treeData={toDataNodes(processTree, (item) => {
            if (item.key.startsWith('process_')) onOpenProcess(item.key);
          })}
          checkedKeys={context.processIds}
          onCheck={(_, info) => {
            const key = String(info.node.key);
            if (key.startsWith('process_')) onProcessToggle(key);
          }}
          onDoubleClick={(_, node) => {
            const key = String(node.key);
            if (key.startsWith('process_')) onOpenProcess(key);
          }}
        />
      </AssetSection>
      <AssetSection
        ...
        {roleInventory ? (
          <div className="role-inventory">
            <div className="role-columns">
              <RoleColumn title="制度角色" roles={roleInventory.policy_roles} onSelect={setSelectedRole} />
              <RoleColumn title="审批角色" roles={roleInventory.process_roles} onSelect={setSelectedRole} />
            </div>
          </div>
        ) : (
      ...
      <Space direction="vertical" size={10} className="full-width">
        ...
        {mappings.map((mapping) => (
          <div className="role-mapping-drawer-row" key={mapping.id}>
            ...
            <Select
              value={mapping.process_role}
              options={processOptions}
              onChange={(value) => onRoleMappingUpdate(mapping.policy_role, value)}
            />
            <Tag color={mapping.mapping_type === 'manual' ? 'green' : 'blue'}>
